function [x_plot,y_plot,old_head,img_x,img_y,gm_child,draw_offset, x,y,head_depth,limit_length,min_depth ,max_depth] = set_bounding_box(directory,frame_number,point1,point2,gm_child_old, reinitializing_number,kinect_value)
...
% fname = 'USB-VID_045E&PID_02AE-A00366809492052A';
if(kinect_value ==1)
    fname = strcat('cam1_timestamp/cam1/USB-VID_045E&PID_02AE-A00364A04906127A');
elseif(kinect_value ==2)
    fname = strcat('cam2_timestamp/cam2/USB-VID_045E&PID_02AE-A00367A14179123A');
elseif(kinect_value ==3)
    fname = strcat('cam3_timestamp/cam3/USB-VID_045E&PID_02AE-A00365A09389104A');
else
    fname = strcat('cam4_timestamp/cam4/USB-VID_045E&PID_02AE-A00366809492052A');
end
% fname = 'USB-VID_045E&PID_02AE-A00364A04906127A';

% load(fullfile(dirpath,[fname,'_',num2str(start),'.mat']),'ColorFrame','DepthFrame');

load(fullfile(dirpath,[fname,'_',num2str(start),'.mat']),'DepthFrame');
load(fullfile(dirpath,[fname,'_DepthFrameToColorFrameMapping.mat']));

c_img = imread( fullfile(dirpath,['cam',int2str(kinect_value),'_timestamp/cam',int2str(kinect_value)],int2str(start),'.png'));
...
depthlist = d_img(y(1):y(2),x(1):x(2));
depthlist = depthlist(:);
depthlist(depthlist<1) = [];
depthlist = sort(depthlist,'descend');
min_depth = depthlist(floor(length(depthlist)*3/4));
max_depth = depthlist(floor(length(depthlist)/4));
head_depth = [min_depth (min_depth+max_depth)/2 max_depth];
...
ch_g = single(ch(:,:,2));
ch_b = single(ch(:,:,3));
gm_child = gmmb_em([ch_r(:) ch_g(:) ch_b(:)],'components',2);
if(reinitializing_number==0)
%     do nthing
else
    gm_child.mu = [gm_child.mu gm_child_old.mu];
%     gm_child.sigma(:,:,1:2) = ...;
%     gm_child.sigma(:,:,3:size(gm_child_old.sigma,3)) = gm_child_old.sigma(:,:,1:end);
%    gm_child.sigma = gm_child.sigma(:,:,1:2) ;
    gm_child.sigma = cat(3, gm_child.sigma, gm_child_old.sigma)
    gm_child.weight = [gm_child.weight;gm_child_old.weight];
end
depth_th = 100;
draw_offset = 2;
old_head = zeros(480,640);
for i=1:range%100
    i
    load(fullfile(dirpath,[fname,'_',num2str(start+i),'.mat']),'ColorFrame','DepthFrame');
    c_img = imread( fullfile(dirpath,['cam',int2str(kinect_value),'_timestamp/cam',int2str(kinect_value)],int2str(start+i),'.png'));
    d_img = map_depthframe_to_colorframe(DepthFrame.DepthData.Depth,DepthFrameToColorFrameMapping);
