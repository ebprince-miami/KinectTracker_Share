...
function InteractiveTracking_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    frame_number = 10000;
    kinect_id = 1; % Gyawu: edited from 2 to 1
    valid_view = 0;
    isPlaying = 0;
    model = [];
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    global directory_selected directory frame_number fnamePrefix...
        timestamp_cam1 timestamp_cam2 timestamp_cam3 timestamp_cam4...
        offset DtCMapping kinect_id max_frames bData imhandle fid;
    directory = uigetdir;
    if directory == 0 
        return;
    end
    ...
    %fnamePrefix{2} =
    %strcat(directory,'/cam2/','USB-VID_045E&PID_02AE-A00367A14179123A');%
...
function frameno = GetMaxFrames(cam_id)
    ...
    timestamp_cam1.time = 3500;
    timestamp_cam2.time = 3500;
    timestamp_cam3.time = 3500;
    timestamp_cam4.time = 3500;% added by Gyawu
  
    %eval(['frameno = length(timestamp_cam', num2str(cam_id), '.time);']); %Commented by Gyawu 
    eval(['frameno = timestamp_cam', num2str(cam_id), '.time;']); %Written by Gyawu to replace above line
