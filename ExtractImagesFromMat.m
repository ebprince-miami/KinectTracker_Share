...
basename = {'USB-VID_045E&PID_02AE-A00364A04906127A';
            ...
            'USB-VID_045E&PID_02AE-B00362214481051B'};


%  mkdir(savepath);
errors = [];
for base_idx = bidx
    filelist = dir(num2str(datapath{base_idx})); % Gyawu: added num2str function
   ...
   % system(['cp "', fullfile(datapath{base_idx}, [basename{base_idx},'_DepthFrameToColorFrameMapping.mat']),'" "', fullfile(spath, [basename{base_idx},'_DepthFrameToColorFrameMapping.mat']),'"']);
     system(['copy "',...
         fullfile(datapath{base_idx},...
         [basename{base_idx},...
         '_DepthFrameToColorFrameMapping.mat']),...
         '" "', fullfile(spath,...
         [basename{base_idx},...
         '_DepthFrameToColorFrameMapping.mat'])...
         ,'"']); % Gyawu: Added this and commented previous line
