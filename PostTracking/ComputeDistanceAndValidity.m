function ComputeDistanceAndValidity(bPos3D, mPos3D, bTimestamp, mTimestamp, savepath)
...
[bPos3DInterpolated, bTimestamp] = InterpolatePos3D(bPos3D, bTimestamp, isBabyDataValid);
[mPos3DInterpolated, mTimestamp] = InterpolatePos3D(mPos3D, mTimestamp, isMomDataValid);
...
for i=1:2:size(pos3d,2)
    ...
    if idx == 1
        ...
        babyApproach(idx) = (dist3d(idx - 1) - norm(pos3d(1:3,i)-pos3d(1:3,i+1-2))) / bCaptureTimeDiff(idx - 1);
        momApproach(idx) = (dist3d(idx - 1) - norm(pos3d(1:3,i-2)-pos3d(1:3,i+1))) / mCaptureTimeDiff(idx - 1);
    end
end

load('GroundPlaneMapping.mat','R','t');
bHeightFromFloor = GetHeadHeightWrtFloor(bPos3DInterpolated, R, t);
mHeightFromFloor = GetHeadHeightWrtFloor(mPos3DInterpolated, R, t);

isBabyCloseToMom = dist3d < 900;
isBabyOnTheFloor = bHeightFromFloor < 900;

save(savepath,'bPos3DInterpolated','mPos3DInterpolated','bPos3D','mPos3D','bTimestamp','mTimestamp','babyApproach','momApproach','isBabyDataValid','isMomDataValid','dist3d','bHeightFromFloor','mHeightFromFloor','isBabyCloseToMom','isBabyOnTheFloor');
