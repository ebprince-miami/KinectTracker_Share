function Get3DPosLSQFIT(bfile, mfile, directory, refCamID, range, firstPassName, savename)

load(fullfile(directory, [firstPassName,'.mat']));
bPos3DOrig = bPos3D;
mPos3DOrig = mPos3D;
bPos3D = Get3DPosKalman(bPos3D, bTimestamp);
mPos3D = Get3DPosKalman(mPos3D, mTimestamp);
...
for i=range
    ...
    elseif strcmp(bMat, mMat)
        ...
        t = bPos3DOrig(curIdx, :);
        if sum(t) ~= 0
            pos3D_b = MapFromC2(t, bData(bNearest,1), R_43,t_43,R_41,t_41,R_42,t_42);
            pos3D_b_smoothed = MapFromC2(bPos3D(curIdx, :), bData(bNearest,1), R_43,t_43,R_41,t_41,R_42,t_42);
            % plot centroid
            Im = draw_circle(Im, K(bData(bNearest,1)), pos3D_b, sphereB, 'cyan');
            Im = draw_circle(Im, K(bData(bNearest,1)), pos3D_b_smoothed, sphereB, 'green');
        ...
        t = mPos3DOrig(curIdx, :);
        if sum(t) ~= 0
            pos3D_m = MapFromC2(t, mData(mNearest,1), R_43,t_43,R_41,t_41,R_42,t_42);
            pos3D_m_smoothed = MapFromC2(mPos3D(curIdx, :), mData(mNearest,1), R_43,t_43,R_41,t_41,R_42,t_42);
            % plot centroid
            Im = draw_circle(Im, K(mData(mNearest,1)), pos3D_m, sphereM, 'magenta');
            Im = draw_circle(Im, K(mData(mNearest,1)), pos3D_m_smoothed, sphereM, 'red');
    ...
    elseif isempty(mMat)
        ...
        if sum(t) ~= 0
            ...
            pos3D_b_smoothed = MapFromC2(bPos3D(curIdx, :), bData(bNearest,1), R_43,t_43,R_41,t_41,R_42,t_42);
            % plot centroid
            bIm = draw_circle(bIm, K(bData(bNearest,1)), pos3D_b, sphereB, 'cyan');
            bIm = draw_circle(bIm, K(bData(bNearest,1)), pos3D_b_smoothed, sphereB, 'green');
            time_b = bData(bNearest, 3);
        end
        ...
        if sum(t) ~= 0
            ...
            pos3D_m_smoothed = MapFromC2(mPos3D(curIdx, :), mData(mNearest,1), R_43,t_43,R_41,t_41,R_42,t_42);
            % plot centroid
            mIm = draw_circle(mIm, K(mData(mNearest,1)), pos3D_m, sphereM, 'magenta');
            mIm = draw_circle(mIm, K(mData(mNearest,1)), pos3D_m_smoothed, sphereM, 'red');
            time_m = mData(mNearest, 3);
        end
...
%     
%     save(fullfile(directory, [savename,'.mat']), 'bPos3D', 'bPos3DOrig', 'mPos3D', 'mPos3DOrig' , 'bTimestamp', 'mTimestamp');
end
